...
import ImportUsersExcelModal from "@/modules/admin/components/ImportUsersExcelModal";
import {
  getUsers,
  createUser,
  deleteUser,
...
export default function AdminUserManagementPage() {
  ...
  const handleImportExcel = async (file: File) => {
    try {
      ...
      showToast("Nhập danh sách người dùng từ Excel thành công.");
      void fetchUserData();
      const sumData = await getUserSummary();
      setSummary(sumData);
    } catch (err: unknown) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex gap-2 shrink-0">
          <button
            ...
            className="flex items-center gap-2 h-9 px-4 rounded-md border border-emerald-600 text-emerald-600 hover:bg-emerald-50 text-xs font-bold bg-white shadow-sm transition-colors cursor-pointer shrink-0"
          >
      ...
      {toast && (
        <div
          className={`fixed bottom-4 right-4 px-4 py-3 rounded-lg shadow-lg border text-sm font-medium animate-in slide-in-from-bottom-5 z-50 flex items-center gap-2 ${toast.type === 'success'
