      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-slate-100 relative">
        <div className="flex items-center gap-2 text-slate-500">
          <button
            ...
            title="Text format (Aa)"
          >
            <span className="font-serif font-bold text-base tracking-tighter">
              Aa
            </span>
          </button>
          ...
          {!hideAttachment && selectedFile && setSelectedFile && (
            <div className="ml-2 flex items-center bg-blue-50 border border-blue-200 text-blue-700 text-xs px-2 py-1 rounded-md max-w-[200px]">
              ...
              <button
                ...
                className="hover:text-red-500 hover:bg-blue-100 rounded-full w-4 h-4 flex items-center justify-center transition-colors"
              >
                ✕
              </button>
            </div>
...
const MessageItem = ({
  ...
  return (
    <div
      className={`group relative flex gap-4 py-3 ${isPost ? "px-0" : "px-4 hover:bg-slate-50/50 rounded-lg -mx-4 transition-colors"}`}
    >
          ...
          <div className="mt-1 bg-white border border-[#1868f0] rounded-md shadow-sm overflow-hidden animate-in fade-in duration-200">
            ...
            <div className="flex justify-end gap-2 p-2 border-t border-slate-100 bg-slate-50">
              <button
                ...
                className="px-3 py-1.5 text-sm font-medium text-slate-500 hover:bg-slate-200 rounded transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={() => onEditSubmit(msg.id, isPost, parentPostId)}
                className="px-4 py-1.5 text-sm font-medium bg-[#1868f0] text-white rounded hover:bg-blue-700 transition-colors"
              >
                Save changes
              </button>
            </div>
            ...
            {msg.attachments.map((att) => {
              ...
              if (isImage) {
                return (
                  <div
                    key={att.id}
                    className="relative rounded-lg overflow-hidden border border-slate-200 max-w-sm group/att"
                  >
                    <Image
                      src={att.url}
                      alt={att.name}
              ...
              if (isVideo) {
                return (
                  <div
                    key={att.id}
                    className="relative rounded-lg overflow-hidden border border-slate-200 max-w-sm bg-black group/att"
                  >
                    <video
                      src={att.url}
                      controls
                      className="w-full h-auto max-h-64 outline-none"
                    />
                    <button
                      type="button"
                      onClick={(e) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0 pr-4">
                    <div
                      className={`w-10 h-10 rounded flex items-center justify-center shrink-0 ${fileConfig.bg} ${fileConfig.text}`}
                    >
                      <svg
                        className="w-6 h-6"
                        fill="currentColor"
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-semibold text-slate-900 truncate group-hover/att:text-blue-600 transition-colors">
                        {att.name}
                      </p>
        ...
        {(msg.reactionCount || 0) > 0 && (
          <div className="mt-2 flex">
            ...
            >
              ...
              <span
                className={`text-[10px] font-medium ml-1 ${msg.userReaction ? "text-blue-600" : "text-slate-600"}`}
              >
                {msg.reactionCount}
              </span>
            </div>
        ...
        <div className="absolute top-2 right-0 opacity-0 group-hover:opacity-100 transition-opacity flex items-center bg-white border border-slate-200 rounded-md shadow-sm z-10 px-0.5">
          <div className="relative group/react">
            ...
            >
              <svg
                className="w-4 h-4"
                fill={msg.userReaction ? "currentColor" : "none"}
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
          ...
          <div className="relative">
            ...
            {activeMenuId === msg.id && (
              <div
                ref={menuRef}
                className="absolute right-0 top-full mt-1 w-40 bg-white rounded-lg shadow-lg z-20 py-1 animate-in zoom-in-95 duration-100 border border-slate-200"
              >
                <button
                  onClick={() => {
                ...
                {msg.isMe && (
                  <>
                    ...
                    <button
                      onClick={() =>
                        handleDeleteClick(msg.id, isPost, parentPostId)
                      }
...
export default function TeamPostsTab({
  ...
  const [activeInputPostId, setActiveInputPostId] = useState<string | null>(
    null,
  );
    ...
    try {
      ...
      const mappedPosts: Post[] = data.map((p: ApiPost) => {
        const mappedAttachments =
          p.attachments?.map((att: ApiAttachment) => ({
            id: att.id,
            name: att.fileName,
        ...
        const avatar =
          ...
          null;
        const rawReact =
          p.userReaction || p.myReaction || p.reactionType || null;
        const userReact = rawReact ? String(rawReact).toUpperCase() : null;
        return {
          id: p.id,
          senderName: name,
          senderAvatar: avatar,
      ...
      setPosts((prevPosts) => {
        return mappedPosts.map((newPost) => {
          // Tìm xem bài viết này trước đó có đang mở comment không
          const existingPost = prevPosts.find((p) => p.id === newPost.id);
          // Nếu có, bê nguyên cái danh sách comment (replies) cũ ráp vào data mới
          if (existingPost && existingPost.replies.length > 0) {
            return { ...newPost, replies: existingPost.replies };
          }
          return newPost;
        });
      });
    ...
    async (postId: string) => {
      try {
        ...
        const mappedComments = comments.map((c: ApiPost) => {
          ...
          const rawReactComment =
            c.userReaction || c.myReaction || c.reactionType || null;
          const userReact = rawReactComment
            ? String(rawReactComment).toUpperCase()
            : null;
  ...
  const handleOpenComment = (postId: string) => {
    ...
    if (!expandedPostIds.includes(postId))
      toggleExpandPost(postId).catch(console.error);
    ...
    try {
      ...
      alert(
        err?.response?.data?.message ||
          "Có lỗi xảy ra khi gửi. Vui lòng kiểm tra lại mạng hoặc dung lượng file đính kèm!",
      );
    } finally {
        ...
        setPosts(
          posts.map((p) => {
            if (p.id === parentPostId) {
              return {
                ...p,
                replies: p.replies.filter((r) => r.id !== idToDelete),
  ...
  const handleReaction = async (
    ...
    try {
      await httpService.post(
        `/interact/reactions?targetId=${id}&targetType=${targetType}&reactionType=${reactionType}`,
      );
        ...
        return prevPosts.map((p) => {
          if (isPost && p.id === id) {
            ...
            const isChangingReaction =
              p.userReaction && p.userReaction !== reactionType;
            ...
            return {
              ...
              replies: p.replies.map((r) => {
                if (r.id === id) {
                  ...
                  const isChangingReaction =
                    r.userReaction && r.userReaction !== reactionType;
  ...
  const handleEditSubmit = async (
    ...
    try {
      ...
      if (isPost) {
        setPosts(
          posts.map((p) => (p.id === id ? { ...p, text: editInputValue } : p)),
        );
      } else if (parentPostId) {
        setPosts(
          posts.map((p) => {
            if (p.id === parentPostId) {
              return {
                ...p,
                replies: p.replies.map((r) =>
                  r.id === id ? { ...r, text: editInputValue } : r,
                ),
              };
            }
            return p;
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 mt-4 mb-3 bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
          ...
          <div className="flex gap-3 w-full sm:w-auto">
            <select
              ...
              className="flex-1 sm:w-auto px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-sm outline-none focus:border-[#1868f0]"
            >
                      ...
                      <div key={comment.id} className="space-y-1">
                        ...
                        {post.replies
                          ...
                          .map((reply) => (
                            <div
                              key={reply.id}
                              className="ml-12 border-l-2 border-slate-100 pl-4 relative mt-1"
                            >
                              <MessageItem
                                msg={reply}
                                isPost={false}
