...
export default function TeamFilesTab() {
  ...
  const { userEmail, classId, isLoaded } = useAppContext();
        ...
        <div className="flex flex-col md:flex-row items-center justify-between mb-6 border-b border-slate-200 pb-4 gap-4">
          <div className="flex items-center gap-3">
            <div className="relative" ref={uploadButtonRef}>
              <button onClick={() => setIsUploadMenuOpen(!isUploadMenuOpen)} disabled={isUploading}
                className="bg-[#1868f0] hover:bg-blue-700 text-white font-medium text-sm flex items-center gap-2 px-4 py-2 rounded-md shadow-sm">
            ...
            <button onClick={handleDeleteAll} disabled={isDeletingAll} className="text-red-500 hover:text-red-700 text-sm flex items-center gap-2 px-3 py-2 rounded-md hover:bg-red-50">
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm pb-10 min-h-[400px]">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredFiles.length === 0 ? <tr><td colSpan={5} className="text-center py-12 text-slate-400">No files found.</td></tr> :
        ...
        <div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            ...
            <div className="w-full h-2 bg-slate-100 rounded-full mb-6 overflow-hidden flex">
              <div className="h-full bg-blue-600" style={{ width: `${(storageStats.docs / (storageStats.totalUsed || 1)) * 100}%` }}></div>
              <div className="h-full bg-red-500" style={{ width: `${(storageStats.media / (storageStats.totalUsed || 1)) * 100}%` }}></div>
            </div>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span>Documents</span><span className="font-bold">{formatBytes(storageStats.docs, 0)}</span></div>
              <div className="flex justify-between"><span>Media</span><span className="font-bold">{formatBytes(storageStats.media, 0)}</span></div>
            </div>
          </div>
        </div>
