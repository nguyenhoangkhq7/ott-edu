/**
 * Chat Sidebar Data Mapping Utilities
 * Extracts media, files, and links from chat messages and maps to UI types
 */

import { ApiMessage } from '@/modules/chat/types';
 ...
 * BULLETPROOF: Handles null/undefined attachments with optional chaining
 */
export function extractMediaItems(messages: ApiMessage[]): MediaItemUI[] {
  ...
  (messages ?? []).forEach((msg) => {
    try {
      // Bulletproof: use optional chaining and fallback to empty array
      ...
      attachments.forEach((attachment) => {
        ...
        if ([...imageExtensions, ...videoExtensions].includes(fileExt)) {
          mediaItems.push({
            ...
            messageId: msg?._id ?? 'unknown',
            senderId: msg?.senderId ?? 'unknown',
            senderName: msg?.senderId ?? 'unknown', // Will be populated from user context
            timestamp: msg?.createdAt ? new Date(msg.createdAt) : new Date(),
          });
        }
      });
    } catch (error) {
      console.error('[extractMediaItems] Error processing message:', error);
...
export function extractFileItems(messages: ApiMessage[]): FileItemUI[] {
  ...
  (messages ?? []).forEach((msg) => {
    try {
      ...
      console.error('[extractFileItems] Error processing message:', error);
      // Continue with next message on error
    }
  });
 ...
 * BULLETPROOF: Handles null/undefined content with optional chaining
 */
export function extractLinkItems(messages: ApiMessage[]): LinkItemUI[] {
  ...
  (messages ?? []).forEach((msg) => {
    try {
      ...
      console.error('[extractLinkItems] Error processing message:', error);
      // Continue with next message on error
    }
  });
 ...
 * BULLETPROOF: Handles null/undefined participants with optional chaining
 */
export function populateSenderNames<T extends { senderId: string; senderName: string }>(
  ...
  try {
    ...
    return itemsArray.map((item) => {
      try {
        const participant = participantsArray.find((p) => p?._id === item?.senderId);
        return {
          ...item,
          senderName: participant?.fullName || item?.senderId || 'Unknown',
        };
      } catch (error) {
        console.error('[populateSenderNames] Error mapping item:', error);
        return item;
      }
    });
  } catch (error) {
    console.error('[populateSenderNames] Error processing items:', error);
    return items ?? [];
  }
}
