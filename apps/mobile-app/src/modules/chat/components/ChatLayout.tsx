...
import { View, StyleSheet, Modal, Pressable, Text, DeviceEventEmitter } from 'react-native';
...
import { ChatMode, Conversation, Message, User, Attachment } from '../types';
import {
  fetchConversations,
  fetchMessages,
  sendMessage,
...
export const ChatLayout: React.FC<ChatLayoutProps> = ({ currentUserId }) => {
  ...
  const loadConversations = useCallback(async () => {
    ...
    try {
      ...
      // 🚀 TẠO BIẾN TẠM ĐỂ HỨNG ID MONGODB CHUẨN XỊN (Sửa lỗi mất tên 1-1)
  ...
  useEffect(() => {
    ...
    const setupSocket = async () => {
      ...
      // Typing Socket Listeners (From Team)
      ...
      // Call Socket Listeners (From Team)
      ...
      const handleCallEnded = ({ conversationId }: { conversationId: string; endedByUserId?: string; reason?: string }) => {
      ...
      // 🚀 LOA PHÓNG THANH & REALTIME GROUP/FRIEND (From You)
  ...
  const handleSendMessage = async (
    ...
    // 🚀 GIỮ FIX LỖI TÌM SAI NGƯỜI NHẬN (Bằng chatMongoId)
    const targetReceiver =
      draftReceiver ||
      activeConversation?.participants.find((p) => p.id !== chatMongoId) ||
      null;

    if (!activeConversation && !targetReceiver) return;
    if (!activeConversationId && !targetReceiver) return;

    const optimisticConversationId = activeConversationId || `draft_${targetReceiver?.id || "unknown"}`;

    const optimisticMessage: Message = {
      id: `optimistic_${Date.now()}`,
      conversationId: optimisticConversationId,
      // 🚀 GIỮ FIX LỖI SENDER ID (Bằng chatMongoId)
      senderId: chatMongoId, 
      content: text,
    ...
    try {
      ...
      if (activeConversationId) {
        ...
        const refreshed = await fetchConversations(chatMongoId);
        setConversations(refreshed);
        const createdConversation = refreshed.find(
          (conv) =>
            conv.type === 'private' &&
            conv.participants.some((p) => p.id === chatMongoId) &&
  ...
  const handleDeclineIncomingCall = useCallback(() => {
    ...
    const activeType = activeConversation?.id === conversationId ? activeConversation.type : undefined;
